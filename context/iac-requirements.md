...
The todo-service is a full-stack task management application with a React SPA frontend and a stateless Node.js/Express backend that provides CRUD endpoints for todos. The backend currently uses an in-memory store. For the golden path, the frontend and backend will be containerized and deployed as two containers on AWS ECS Fargate behind an Application Load Balancer, with logs sent to CloudWatch and Container Insights enabled for metrics.
